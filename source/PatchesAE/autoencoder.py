...
class Autoencoder(L.LightningModule):
    ...
    @staticmethod
    def reconstruct_image(reconstruction_mask, patches):
        """
        Reconstruct image from patches
        
        params:
            - reconstruction_mask: torch.Tensor, zero for masked patch, one for reconstructed patch
        """
        patch_size = patches.shape[3]
        H, W = reconstruction_mask.shape
        H, W = H * patch_size, W * patch_size   # final image dimensions
        ...
        for i in range(reconstruction_mask.shape[0]):
            for j in range(reconstruction_mask.shape[1]):
                # insert the patch where it has been extracted
        ...
        image = (image - image.min()) / (image.max() - image.min() + 1e-5)  # normalize between 0 and 1
        image = image.permute(2, 0, 1)  # use torch convention [C, H, w]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # plot the first patch reconstruction outcome (just for visualization)
        if batch_idx == 0:
