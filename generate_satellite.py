...
def main(streetview_path):
    ...
    semantic = torch.zeros(sv_img.shape[1], sv_img.shape[2])
    ...
    z = torch.cat([z] * 2)
    for t in tqdm(reversed(range(scheduler.timesteps)), total=cfg['timesteps']):
        z = scheduler.scale_model_input(z, t)
        with torch.no_grad():
            pred = ldm(z.to('cuda'), conditioning, torch.Tensor([t]).to('cuda'))
            
        cond, uncond = pred.chunk(2)
        pred = uncond + 3.0 * (cond - uncond)
        z = scheduler.step(pred, t, z).prev_sample
    
    with torch.no_grad():
        sat = (((vae.decode(z).sample.clamp(-1.0, 1.0)) + 1) * 127.5).int()[0].permute(1, 2, 0)
